...
export const omniExecutor: Executor = (
  ...
  return new Promise((c, e) => {
    ...
    const commands = vscode.workspace
      .getConfiguration("handydandy-notebook")
      .get("dispatch") as Record<string, [string, string[]]>;
    if (!commands[language]) {
      logger(
        `Your Handy Dandy Notebook cannot execute ${
          language || `_undefined lang_`
        } cells. Try adding an entry to \`handydandy-notebook.dispatch\` in settings.`
      );
      return c(undefined);
    }
...
export const makeNotebookController = (
  controllerId: string,
  notebookId: string,
  label: string,
  executor: Executor
        ...
        (s) =>
          execution.appendOutput(
            new vscode.NotebookCellOutput([
              vscode.NotebookCellOutputItem.text(s),
            ])
          ),
        execution.token
      );
